...
# Cached wrapper for cover retrieval
...
for month_code in sorted(grouped[year].keys(), reverse=True):
    ...
    exp_key = f"month_open_{year}_{month_code}"
    expanded_default = (year == str(datetime.now().year)
                        and month_code == datetime.now().strftime("%m"))

    with st.expander(f"🗓️ {month_name} ({len(month_books)} book{'s' if len(month_books) != 1 else ''})",
                     expanded=expanded_default):

        # remember which months were opened (for persistence between reruns)
        if st.session_state.get(exp_key) is None:
            st.session_state[exp_key] = expanded_default
        if st.session_state[exp_key] is False and expanded_default:
            st.session_state[exp_key] = True
        if st.session_state[exp_key] is False:
            # month not opened yet — skip heavy rendering
            st.caption("⏳ Expand this month to load book details...")
            continue

        # --- Month content (only runs after expansion) ---
